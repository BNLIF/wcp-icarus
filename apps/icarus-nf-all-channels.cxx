...
{
  if (argc < 2) {
    cerr << "usage: icarus-nf-all-channels #filename #outfile" << endl;
  }else{

    WCPICARUS::ExecMon em("starting");
    ...
    double lambda = 0.8;//0.05; // regularization strength ...
    //    double lambda1 = 0.05*400;//0.05; // regularization strength ...
    const int nsize = 4096;
    double lambda_n = nsize*n_channels*10000; //(nsize-no_list.size())*32; // noise part ...

      
    
    Eigen::VectorXd b(n_channels*nsize+nsize); // measurement matrix
    Eigen::VectorXd b_2nd(n_channels*nsize+nsize); // measurement matrix
    Eigen::VectorXd temp(nsize);
    Eigen::SparseMatrix<double> A(n_channels*nsize+nsize,n_channels*nsize+nsize); // cofficient ...
    Eigen::SparseMatrix<double> A_2nd(n_channels*nsize+nsize,n_channels*nsize+nsize); // cofficient ...
    Eigen::VectorXd result_init(n_channels*nsize+nsize); // solution ...
    Eigen::VectorXd result(n_channels*nsize+nsize); // solution ...
    ...
    tripletList.reserve(nsize*n_channels*4);
    tripletList_2nd.reserve(count_non_zero * (n_channels+3));
	...
	for (Int_t k=0;k!=nticks;k++){
	  ht->SetBinContent(k+1,hu[2]->GetBinContent(j+1,k+1));
	}
	h_real = ht->FFT(0,"RE");
	h_imag = ht->FFT(0,"IM");
	...
	for (Int_t k=0;k!=nsize;k++){
	  ...
	  tripletList.push_back(T(nsize*j+k,nsize*n_channels+k,1.0));

	  tripletList_2nd.push_back(T(nsize*j+k,nsize*j+k,1.0 + lambda));
	  tripletList_2nd.push_back(T(nsize*j+k,nsize*n_channels+k,1.0));
	  
	  
	  b(n_channels*nsize+k) += temp(k);
	  tripletList.push_back(T(nsize*n_channels+k,nsize*j+k,1));
	  
	  if (no_list.find(k) == no_list.end()){
	    b_2nd(n_channels*nsize+k) += temp1(k);
	    // last equation ...
	    for (Int_t q=0;q!=nsize;q++){
	      if (cov(k,q)!=0) {
		tripletList_2nd.push_back(T(nsize*n_channels+k,nsize*j+q,cov(k,q)));
	      }
	    }
	  }else{
	    b_2nd(n_channels*nsize+k) += temp(k);
	    tripletList_2nd.push_back(T(nsize*n_channels+k,nsize*j+k,1));
	  }
	}
	delete h_real;
	delete h_imag;
      ...
      // last piece ...
      for (Int_t k=0;k!=nsize;k++){
	tripletList.push_back(T(nsize*n_channels+k,nsize*n_channels+k, n_channels));
	
	if (no_list.find(k) == no_list.end()){
	  tripletList_2nd.push_back(T(nsize*n_channels+k,nsize*n_channels+k,lambda_n + n_channels*cov(k,k)));
	}else{
	  tripletList_2nd.push_back(T(nsize*n_channels+k,nsize*n_channels+k, n_channels));
	}
	
       	for (Int_t q=k+1;q!=nsize;q++){
	  if (no_list.find(q) == no_list.end()){
	    if (cov(q,k) != 0){
	      tripletList_2nd.push_back(T(nsize*n_channels+q,nsize*n_channels+k,n_channels*cov(q,k)));
	      tripletList_2nd.push_back(T(nsize*n_channels+k,nsize*n_channels+q,n_channels*cov(k,q)));
	    }
	  }
	}
      }
      ...
      solver_2nd.setMaxIterations(500);
      solver_2nd.setTolerance(1e-6);
	...
	for (Int_t k=0;k!=nsize;k++){
	  hu1[2]->SetBinContent(j+1,k+1, fb->GetBinContent(k+1));
	}
      }
